"""
    Updates
    11/12/2020
    Enable custom outer diameter definition. It is allowed to define a single value for the outer diameters of all disks.
    The definition should be performed when SimManipulatorAdamModel is instantiated 
"""
...
class SimManipulatorAdamModel:
    ...
    def _import_CADs(self, disk_models: List[DiskModel], disk_outer_diameter: float = None):
        success = True
        for i, model in enumerate(disk_models):
            ...
            disk_geometry = SimDiskGeometry.from_base_geometry(
                base_disk_geometry=model.disk_geometry,
                tendon_models=model.tendon_models,
                outer_diameter=disk_outer_diameter
            )
    ...
    def _generate_contacts(self, disk_models: List[DiskModel], config: ContactConfig):
        for i in range(len(disk_models)-1):
            self.socket.create_contact(
                ...
                config.friction_coef, # Same value in both static or dynamic scenarios
                config.friction_vel,
                config.friction_vel, # Same value in both static or dynamic scenarios
            )
    ...
    def _generate_external_loads(self,
        ...
        for i, el in enumerate(external_loads):
            if el.is_attached_to_disk:
                self.socket.modify_part_rigid_body(
                    self.name_gen.disk_part_name(el.disk_index))
            else:
                self.socket.modify_part_rigid_body(
                    self.name_gen.ground_part_name)
            self.socket.create_marker(
                ...
                el.location,
                relative_to=self.name_gen.base_disk_bottom_marker_name if el.is_location_global else self.name_gen.disk_part_name(
                    el.disk_index)
            )
            self.socket.create_general_force(
                self.name_gen.force_external_force_name(i),
                self.name_gen.external_load_marker_name(i),
                self.name_gen.ground_part_name if el.is_attached_to_disk else self.name_gen.disk_part_name(
                    el.disk_index),
                ref_marker_name=self.name_gen.ground_base_disk_bottom_marker_name if el.is_force_vector_global else self.name_gen.disk_center_marker_name(
                    el.disk_index),
    ...
    def run_sim(self,
        ...
        if not self._import_CADs(disk_models, disk_outer_diameter=self.manipulator_model.outer_diameter):
        ...
        for el in external_loads:
            if el.disk_index > self.manipulator_model.num_joints:
                raise ValueError(
                    "External load is applied on non-existing disk")
